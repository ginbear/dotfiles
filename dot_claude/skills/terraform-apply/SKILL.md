...
- 対象の plan が実行済みで、ユーザーが内容を確認済みであること
...
cd <実行ディレクトリの絶対パス> && <terraform or terragrunt> apply 2>&1 | tee -a "$LOG_FILE"
```
...
- `No changes` が確認できれば OK（このログの保存は不要）
- **差分が残っている場合**: ユーザーに報告し、次のステップに進まない
